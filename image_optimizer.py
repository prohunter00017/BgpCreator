...
class ImageOptimizer:
    ...
    def _load_image_config(self):
        """Load image configuration from seo_config.json"""
        try:
            with open("seo_config.json", 'r', encoding='utf-8') as f:
                config = json.load(f)
    ...
    def _optimize_hero_images(self):
        ...
        # Look for original hero images in static folder
        hero_image_path = os.path.join(self.input_dir, "hero-image.webp")
        ...
        if os.path.exists(hero_image_path):
            ...
            # Copy the original image with SEO-optimized name (no responsive generation)
            seo_image_path = os.path.join(self.output_dir, f"{seo_filename}.webp")
            shutil.copy2(hero_image_path, seo_image_path)
            print(f"  ✅ Copied to SEO-optimized name: {seo_filename}.webp")
    ...
    def _get_site_name(self):
        """Get site name from seo_config.json"""
        try:
            with open("seo_config.json", 'r', encoding='utf-8') as f:
                config = json.load(f)
        ...
        try:
            with Image.open(favicon_path) as img:
                ...
                for size in favicon_sizes:
                    ...
                    # Save as PNG
                    png_path = os.path.join(self.output_dir, f"favicon-{size}x{size}.png")
                    resized.save(png_path, "PNG", optimize=True)
                    
                    # Save as ICO for specific sizes
                    if size in [16, 32, 48]:
                        ico_path = os.path.join(self.output_dir, f"favicon-{size}x{size}.ico")
                        resized.save(ico_path, "ICO")
                
                # Copy original favicon
                shutil.copy2(favicon_path, os.path.join(self.output_dir, "favicon.ico"))
                
                print(f"  ✅ Generated favicon in {len(favicon_sizes)} sizes")
